...
import { isAfterDay, isBeforeDay, isSameDay, startOfDay } from '../../utils/date';
...
const getDateInputParts = (value: string) => {
  ...
  if (secondDayDigit) {
    const isValidTwoDigitDay = firstDayDigit === '0'
  ...
  if (firstMonthDigit === '0') {
    return {
      day,
      month: Number(secondMonthDigit) > 0 ? `0${secondMonthDigit}` : firstMonthDigit,
  ...
  if (firstMonthDigit === '1') {
    ...
    return {
      day,
      month: isValidTwoDigitMonth ? `${firstMonthDigit}${secondMonthDigit}` : `0${firstMonthDigit}`,
...
const parseInputDate = (value: string): Date | null => {
  const { day: dayPart, month: monthPart, year: yearPart } = getDateInputParts(value);
...
const isSameInputDate = (firstDate: DateInputValue, secondDate: DateInputValue): boolean => {
...
const emitChange = (value: DateInputValue, nextInputValue: string, isValid: boolean): void => {
  props.onChange?.({
    value,
